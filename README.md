## GameArena 🎮

GameArena is a **responsive gaming landing page** built with **HTML5, CSS3, and Bootstrap**.  
It showcases a modern eSports brand with hero section, latest games carousel, services, partners logos animation, team members, and a contact form.

### Live Demo (GitHub Pages)
- **GitHub Pages**: [`https://mohammeddev1.github.io/gamearena-css/`](https://mohammeddev1.github.io/gamearena-css/)

### Features
- **Hero section** highlighting "The Ultimate Gaming Experience"
- **Our Games** carousel built with Bootstrap
- **What We Do** section for services (PC, mobile, AR/VR, etc.)
- **Animated partners/logo strip**
- **Our Team** grid with player cards
- **Contact Us** section with form and community/social links
- **Responsive layout** using Bootstrap grid and custom CSS

### Tech Stack
- **HTML5**
- **CSS3** (`main.css`, `media.css` for responsiveness)
- **Bootstrap 5** (`bootstrap.min.css`, `bootstrap.bundle.min.js`)
- **Font Awesome** icons

### How to Run Locally
1. Clone the repo:
   ```bash
   git clone https://github.com/mohammeddev1/gamearena-css.git
   ```
2. Open the project folder:
   ```bash
   cd gamearena-css
   ```
3. Open `index.html` in your browser (you can just double‑click it or use a local server).

 


